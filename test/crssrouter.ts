...
describe("CrssToken test", async () => {
  ...
  before("load fixture loader", async () => {
    ...
    pair = (await hre.ethers.getContractAt(pairAddress, JSON.stringify(CrossPairAbi.abi))).connect(owner) as CrossPair;
    ...
    wbnbPair = (await hre.ethers.getContractAt(wbnbPairAddress, JSON.stringify(CrossPairAbi.abi))).connect(
      owner
    ) as CrossPair;
  });
  ...
  it("addLiquidity", async function () {
    ...
    await token0.approve(router.address, ethers.constants.MaxInt256);
    await token1.approve(router.address, ethers.constants.MaxInt256);
    await expect(
      router.addLiquidity(
        token0.address,
        token1.address,
  ...
  async function addLiquidity(token0Amount: BigNumber, token1Amount: BigNumber) {
    await token0.connect(owner).mint(token0.address, token0Amount);
    await token1.connect(owner).mint(token1.address, token1Amount);
