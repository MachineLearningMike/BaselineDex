...
describe("CrssToken test", async () => {
  ...
  async function createPair(tokens: [string, string]) {
    const bytecode = `0x${CrossPairAbi.bytecode}`;
    ...
    const pair = (await ethers.getContractAt(create2Address, JSON.stringify(CrossPairAbi.abi))) as CrossPair;
  ...
  it("setFeeTo", async () => {
    await expect(factory.connect(userA).setFeeTo(userA.address)).to.be.revertedWith("Pancake: FORBIDDEN");
  ...
  it("setFeeToSetter", async () => {
    ...
    expect(await factory.feeToSetter()).to.eq(owner.address);
    await expect(factory.connect(owner).setFeeToSetter(owner.address)).to.be.revertedWith("Cross: FORBIDDEN");
  });
});
